...
log = logging.getLogger('dreamingtree')


class InputDreamingTree(object):
    """A plugin that searches dreamingtree.org
    ...
    def _getval(self, key, val):
        ...
        if isinstance(opts, dict):
            # Translate the input value to the Dreaming Tree (Gazelle) API value
    ...
    def _login(self, user, passwd):
        ...
        r = self.session.post("http://dreamingtree.org/login.php", data=data,
                              allow_redirects=False)
        if r.status_code != 302 or r.headers.get('location') != "index.php":
            raise PluginError("Failed to log in to dreamingtree.org")

        accountinfo = self._request('index')

        self.authkey = accountinfo['authkey']
        self.passkey = accountinfo['passkey']
        log.info("Logged in to dreamingtree.org")
    ...
    def _request(self, action, page=None, **kwargs):
        ...
        ajaxpage = "http://dreamingtree.org/ajax.php"
        ...
        if r.status_code != 200:
            raise PluginError("dreamingtree.org returned a non-200 status code")
        ...
        try:
            ...
            if json_response['status'] != "success":
                ...
                raise PluginError("dreamingtree.org gave a failure response: "
                                  "'{}'".format(error))
            return json_response['response']
        except (ValueError, TypeError, KeyError) as e:
            raise PluginError("dreamingtree.org returned an invalid response")
        ...
        for result in search_results:
            ...
            for tor in result['torrents']:
                ...
                yield Entry(
                    ...
                    url="http://dreamingtree.org/torrents.php?action=download&"
...
def register_plugin():
    plugin.register(InputDreamingTree, 'dreamingtree', groups=['search'], api_ver=2)
